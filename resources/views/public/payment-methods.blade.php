<x-public-layout :title="__('Payment Methods') . ' — Finix CRM'">
    <div x-data="{
        copy(text, btn) {
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
            ...
            <div class="text-center max-w-3xl mx-auto space-y-6">
                ...
                <h1 class="text-4xl font-black text-slate-900 tracking-tight">{{ __('Payment Methods') }}</h1>
                <p class="text-lg text-slate-500 font-medium leading-relaxed">
                    {{ __('We offer several payment methods to make your purchases easier.') }}
                </p>
            </div>
                            ...
                            @foreach($methodsByCategory->get($categoryKey) as $method)
                                <div class="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:shadow-xl transition flex flex-col justify-between">
                                    <div class="space-y-6">
                                        ...
                                        @if($method->reference_required)
                                            <div class="bg-indigo-50 border border-indigo-100 p-3 rounded-xl text-xs text-indigo-700 font-medium">
                                                {{ __('Please include your order number as the payment reference.') }}
                                            </div>
                                        @endif

                                        @if($method->proof_required)
                                            <div class="bg-slate-50 border border-slate-200 p-3 rounded-xl text-xs text-slate-600 font-medium">
                                                {{ __('A proof of payment will be required to confirm this order.') }}
                                            </div>
                                        @endif

                                        @if($method->requires_confirmation)
                                            <div class="bg-slate-900 text-slate-100 p-3 rounded-xl text-xs font-bold flex items-center gap-2">
                                                ...
                                                {{ __('Manual confirmation required — usually verified within a few hours.') }}
                                            </div>
                                        @endif
